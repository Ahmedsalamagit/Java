...
public class StudentDirectoryTest {
    ...
    public void testAddStudent() {
        ...
        //Test adding second Student
        sd.addStudent("Test", "McTesterson", "tmcktest", EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
        assertEquals(1, studentDirectory.length);
        studentDirectory = sd.getStudentDirectory();
        assertEquals("Test", studentDirectory[1][0]);
        assertEquals("McTesterson", studentDirectory[1][1]);
        assertEquals("tmcktest", studentDirectory[1][2]);
        
        //Test valid Student with < 3 max credits
        sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, 2);
        String [][] studentDir = sd.getStudentDirectory();
        assertEquals(2, studentDir.length);
        assertEquals(FIRST_NAME, studentDir[0][0]);
        assertEquals(LAST_NAME, studentDir[0][1]);
        assertEquals(ID, studentDir[0][2]);
        ...
        try {
            sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, "test", "testies", MAX_CREDITS);
            fail();
        } catch(IllegalArgumentException e)
        {
    ...
    /**
     * Test valid and invalid inputs for the getStudentById method
     */
    @Test
    public void testGetStudentById()
    {
        ...
        assertEquals(s.getFirstName(), "Zahir");
        assertEquals(s.getLastName(), "King");
        assertEquals(s.getId(), "zking");
        assertEquals(s.getEmail(), "orci.Donec@ametmassaQuisque.com");
